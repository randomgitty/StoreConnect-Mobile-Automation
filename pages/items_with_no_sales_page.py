...
class ItemsWithNoSalesPage(BasePage):
    def __init__(self, driver):
        super().__init__(driver) 
    def get_page_title(self):
        title_locator = (AppiumBy.ANDROID_UIAUTOMATOR, 'new UiSelector().description("Item With No Sales")')
    ...
    def click_product_card(self):
        product_card_locator = self.get_product_card_locator("Stock")
        ...
        try:
            ...
            raise AssertionError(f"❌ Failed to click product card with 'Stock': {str(e)}")
        
        card_text = card_element.get_attribute("contentDescription")
        logger.info(f"Card Description: '{card_text}'")

    def complete_item_with_no_sales_flow(self):
        title = self.get_page_title()
        assert title == "Item With No Sales", f"❌ Expected 'Item With No Sales', got '{title}'"
        ...
        logger.info("✅ Completed Items with No Sales flow")